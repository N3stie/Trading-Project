...
function renderTradesTable(trades) {
    ...
    if (trades.length === 0) {
        tbody.innerHTML = `<tr class="empty-row"><td colspan="13">No trades found</td></tr>`;
        return;
    }
    ...
    tbody.querySelectorAll('.action-btn.delete').forEach(btn => {
        btn.addEventListener('click', (e) => deleteTradeAndRefresh(e.target.dataset.id));
    });
...
function editTrade(tradeId) {
    ...
    setTimeout(() => {
        ...
        document.querySelector('#addTradeForm .btn-primary').textContent = 'Update Trade';
        
        if (trade.screenshot && trade.screenshot.startsWith('data:')) {
            document.getElementById('screenshotPreview').innerHTML = `<p>Current screenshot:</p><img src="${trade.screenshot}" style="max-width:200px;border-radius:8px;border:1px solid var(--border-color);">`;
        }
    }, 100);
}
